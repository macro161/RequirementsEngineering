...
\label{table:PSReq-WhyHowWhat} \\ \hline
		...
		\ref{sec:PSReqHow} skyrelis&
		 \ref{sec:PSReqWhat} skyrelis\\ \hline
...
\label{table:PSReq-WhoWhereWhen} \\ \hline
		...
		Programos reikalavimai (Kiekvienai sistemai) &
		 \ref{sec:PSReqWho} skyrelis&
		 \ref{sec:PSReqWhere} skyrelis&
		 \ref{sec:PSReqWhen} skyrelis\\ \hline
...
Sistemos sąsajas su išoriniu pasauliu patogiausia vaizduoti konteskto diagrama:
\begin{figure}[H]
	\centering
...
Pagal konteksto diagramą identifikuoti verslo panaudojamumo atvejai:
...
\end{figure}
\newpage
\subsubsubsection{Įvestys}\label{sec:versloReqWhyInput}

\begin{itemize}
	\item registracijos didesnės rizikos renginiuose (matai - kiekis ir korektiškumas);				                                                             
	\item registracijos viešojo maitinimo įstaigose;                               
	\item informacija apie keliones iš rizikos šalių;                                 
	\item pavojingų šalių sąrašas;                                               
	\item asmenų ligos istorijos;                                                     
	\item testų rezultatai;                                                          
	\item saviizoliaciją patvirtinantys vietos nustatymo duomenys                    
	\item saviizoliaciją patvirtinanti nuotrauką                                    
	\item atvykusių iš pavojingų šalių žmonių sąrašas                                
	\item masinių renginių registracijos                                            
	\item užsikrėtusiųjų maršrutai                                                
	\item pranešimai apie įtariamus atvejus 
\end{itemize}

\subsubsubsection{Išvestys}\label{sec:versloReqWhyOutput}

\begin{itemize}
	\item statistika;
	\item pranešimas apie priverstinę saviizoliaciją;
	\item pranešimas apie saviizoliacijos pažeidimą;
	\item pranešimas apie riziką;
	\item pranešimas renginių org. apie atvejus;
\end{itemize}

\subsubsubsection{Reguliacija}\label{sec:versloReqRegulation}
...
\subsubsubsection{Įvaizdis}\label{sec:versloReqWhyAssessment}
...
Pagrindinės problemos aptariamos įžangos probleminėje srityje.

\subsubsubsection{Vizija}\label{sec:versloReqWhyVision}
Tapti pagrindiniu viruso plitimo visuomenėje valdymo įrankiu specialistams.

\subsubsubsection{Misija}\label{sec:versloReqWhyMission}
...
\subsection{Kaip?}\label{sec:PSReqHow}
	...
	\subsubsection{Programų sistemos instaliuojamumo reikalavimai}
		...
		Maksimalus procentas failų kuriuos reiktų pakeisti instaliuojant sistemą yra 0,01\%.
	\subsubsection{Programų sistemos atitikimo keliamumo standartams reikalavimai apibūdina}
		Keliamumo reikalavimai atitinką apibrėžtą vidinį standartą pagal ISO.
	\subsubsection{Programų sistemos pakeičiamumo reikalavimai}
		Sistema nauja, todėl nėra senos sistemos kurią būtų galima pakeisti.
	\subsubsection{Programų sistemos analizuojamumo reikalavimai}
		Trigdžio priežastis sistemoje turi būti išsiaiškinta per 20 žmogaus darbo valandų.
	\subsubsection{Programų sistemos keičiamumo reikalavimai}
		Trigžio priežastis sistemoje turi būti pašalinta per 20 žmogaus darbo valandų.
	\subsubsection{Programų sistemos stabilumo reikalavimai}
		Mažiausias vidutinis laikas kurį sistema bus įtakota dėl atsitiktinio neteisingo pakeitimo yra 4 valandos.
	\subsubsection{Programų sistemos testuojamumo reikalavimai}
		Testai sistemoje nevykdomi.
	\subsubsection{Programų sistemos tvarkomumo reikalavimai}
		Sistema turi būti atkuriama per 1 žmogaus darbo valandą.
	\subsubsection{Programų sistemos pakartotino panaudojamumo reikalavimai}
		Sistema negali būti perpanaudota.
...
\begin{itemize}
	...
	\item Visas disponavimas duomenimis turi atitikti Lietuvos Respublikos ir Europos Sąjungos jautrių duomenų disponavimo įstatymus.
\end{itemize}

\subsection{Kas?}\label{sec:PSReqWho}
Sistema atitinka žmogaus ir kompiuterio sąveikos euristikas.
\subsection{Kur?}\label{sec:PSReqWhere}
\textbf{Minimalūs kompiuterio platformos reikalavimai:}
\begin{enumerate}
	\item OS: Windows 7 arba naujesnė
	\item Procesorius: Intel(R) Core(TM) i5-2520M arba panašaus našumo procesorius
	\item Atmintis: 4 GB RAM
	\item Grafika: GeForce GTX 500 serija arba panašus
	\item Kietojo disko talpa: 3 GB laisvos vietos
	\item Microsoft Internet Explorer 10+ - Windows
\end{enumerate}

\textbf{Rekomenduojami kompiuterio platformos reikalavimai:}
\begin{enumerate}
	\item OS: Windows 7 arba naujesnė
	\item Procesorius: Intel(R) Core(TM) i5 2.6GHz arba panašaus našumo procesorius
	\item Atmintis: 4 GB RAM
	\item Grafika: GeForce GTX 700 serija arba panašus
	\item Kietojo disko talpa: 3 GB laisvos vietos
	\item Microsoft Internet Explorer 11+ - Windows
\end{enumerate}

\subsection{Kada?}\label{sec:PSReqWhen}
\textbf{Našumo reikalavimai:}
\begin{enumerate}
	\item Didžiausia leistina tinklapio sistemos apkrova yra 10000 vartotojų, prisijungusių vienu metu.
	\item Tinklapio didžiausias leistinas reakcijos laikas, neįvertinant interneto greičio, turi būti ne didesnis kaip 1 sekundė, o užklausos vykdymo laikas ne didesnis nei 2 sekundės. Netenkinant šio kriterijaus, tinklapio algoritmas turi būti spartinimas.
	\item Viena transakcija negali trukti ilgiau negu 0,1 sekundės.
\end{enumerate}
